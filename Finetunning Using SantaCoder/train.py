...
# Define the maximum buffer size as a constant
MAX_BUFFER_SIZE = 1024 * 3.6 * 1024  # seq_length * chars_per_token * num_of_sequences
...
def estimate_chars_token_ratio(dataset, tokenizer, data_column, num_samples=400):
    total_characters, total_tokens = 0, 0
    for _, example in tqdm(zip(range(num_samples), iter(dataset)), total=num_samples):
...
class ConstantLengthDataset(IterableDataset):
    def __init__(
        ...
        self.eos_token_id = tokenizer.eos_token_id or args.eos_token_id
    ...
    def __iter__(self):
        ...
        while more_examples:
            ...
            for tokenized_input in tokenized_inputs:
                ...
                all_token_ids.extend(tokenized_input + [self.eos_token_id])
            examples = [example for example in [all_token_ids[i:i + self.seq_length] for i in range(0, len(all_token_ids), self.seq_length)] if len(example) == self.seq_length]
...
def create_datasets(tokenizer, args):
    ...
    chars_per_token = estimate_chars_token_ratio(train_data, tokenizer, args.data_column)
